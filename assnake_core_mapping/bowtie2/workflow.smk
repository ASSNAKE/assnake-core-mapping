...
rule create_seq_set_index_bowtie2:
    ...
    output:
        ref_index = os.path.join(index_dir, 'bowtie2/{path}/{seq_set_id}/index.bt2')
    params:
    ...
    threads: 8
    conda: 'env.yaml'
    shell: ('''bowtie2-build --threads {threads} --seed {params.seed} {input.ref} {params.prefix} > {log} 2>&1"''')
...
rule map_on_ref_bowtie2:
    ...
    threads: 8
    conda: 'env.yaml'
    shell: 'bowtie2 -x {input.refer}/reference -1 {input.f} -2 {input.r} -U {input.unf},{input.unr} -S {output.sam} -p {threads} -D 20 -R 5 -N 1 -L 20 -i S,1,2.50 > {output.sam}_mapp_log.txt 2>&1'